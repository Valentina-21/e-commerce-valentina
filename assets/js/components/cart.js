import modal from './modal.js';
 const modalFunctions = modal();

function cart( db, printProducts, showModal, showModalNoArticles,
  thanks, confirmation, deleteModal) {
  let cart = []
  ...
  cartDOM.addEventListener('click', function (e) {
    if (e.target.closest('.article--minus')) {
      ...
      removeFromCart(id)
      printCart()
    } else if (e.target.closest('.article--plus')) {
      const id = +e.target.closest('.article--plus').dataset.id
      addToCart(id)
      printCart()
    } else if (e.target.closest('.remove-from-cart')) {
      const id = +e.target.closest('.remove-from-cart').dataset.id
      const deleteAction = modalFunctions.deleteModal();
      const isConfirmed = deleteAction;
      if (isConfirmed) {
        deleteFromCart(id);
        printCart()
      }  
